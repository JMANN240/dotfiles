...
gbranch() {
	git branch 2> /dev/null | perl -ne 'print $1 if /^\* (.+)$/'
}
...
gdiff() {
	if [ ! -z "$(gbranch)" ]; then
		GIT_ROOT=$(groot);
		git diff --name-only master...$(gbranch) | perl -pe "s|^|$GIT_ROOT/|";
	else
		return 1;
	fi
}
	...
	if [ ! -z "$(git branch 2> /dev/null | perl -ne 'print $1 if /^\* (.+)$/')" ]; then
		if [ ! -z "$(git status -s --porcelain)" ]; then
			...
			FILES=($(
				...
				perl -ne "print if s|(.M\|\?\?\|UU\|AA) (.+)|$GIT_ROOT/\$2|" |
				fzf -0 -m --reverse --ansi --bind pgdn:preview-page-down,pgup:preview-page-up --preview='git diff --color -- {}' --preview-window='up,80%,wrap'
			));
			if [ -z "$FILES" ]
			then
				return 0
...
# Really fancy git diff! Requires git (of course), fzf, and perl.
gd() {
	git diff $(git merge-base master HEAD) --name-only |
	perl -pe "s|(.+)|$(groot)/\$1|" |
	fzf -0 -m --reverse --ansi --bind pgdn:preview-page-down,pgup:preview-page-up --preview='git diff $(git merge-base master HEAD) --color -- {}' --preview-window='up,80%,wrap'
}
...
gbr() {
	if [ ! -z "$(gbranch)" ]; then
		...
		for DEAD_BRANCH in ${DEAD_BRANCHES[@]}; do
			WORKTREE_PATH=$(git worktree list | perl -ne "print if s|(.+) +.{9} \[$DEAD_BRANCH\]|\$1|")
...
ws() {
	git diff --unified=0 | perl -ne 'print if s/^\+(?!\+\+)(.+)$/$1/' | grep -q -P '[\t ]+$'
}

commitfiles() {
	git log --oneline --color --decorate | fzf --ansi --no-sort --layout=reverse-list --preview 'git show --name-only --color {1}'
}

fzflog() {
	git cherry -v master | tac | fzf --ansi --bind pgdn:preview-page-down,pgup:preview-page-up --preview 'source ~/.utils.sh; showsplit {2}'
}

filehist() {
	git log --format='format:%Cgreen%h%Creset (%Cblue%an%Creset, %Cred%ai%Creset) %s' -- $1 | fzf --ansi --reverse --bind pgdn:preview-page-down,pgup:preview-page-up --preview "git diff --color {1}^ {1} -- $1" --preview-window='wrap'
}

commitgrep() {
	GIT_BRANCH=$(git branch 2> /dev/null | perl -ne 'print $1 if /^\* (.+)$/');
	git log --oneline --color --decorate | fzf --ansi --no-sort --layout=reverse-list --preview "git checkout {1} 2> /dev/null ; grep -P -r -I '$1' | perl -pe 's|^((?:[^\/]+\/)*[^\/]+\.\w+):\s*(.*)$|\1: \2|'" --preview-window='up,80%,wrap';
	git checkout $GIT_BRANCH 2> /dev/null;
}

showcommits() {
	git log --oneline --color --decorate | fzf --ansi --no-sort --layout=reverse-list --preview "git show {1} --color" --preview-window='up,80%,wrap';
}
